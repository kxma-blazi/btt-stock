  ...
  try {
    ...
    if (lastAssetRow > 1) {
      ...
      const validAssets = rawAssets.filter(
        (row) => String(row[3]).trim() !== "" || String(row[4]).trim() !== "",
      );
...
function saveBatchBorrowData(borrowPayload) {
  ...
  try {
    ...
    if (
      !borrowPayload ||
      !borrowPayload.items ||
      borrowPayload.items.length === 0 ||
      !borrowPayload.borrowerName
    ) {
      return {
        success: false,
        message: "ข้อมูลการยืมไม่ครบถ้วน กรุณาเลือกรายการและระบุชื่อผู้เบิก",
      };
    }
    ...
    logActivity(
      borrower,
      "BATCH_BORROW",
      `เบิกอุปกรณ์รวม ${successCount} รายการ โครงการ: ${project}`,
    );
    sendTelegramAlert(
      `📤 <b>บันทึกการเบิก/ยืมอุปกรณ์ (หลายรายการ)</b>\n👤 ผู้เบิก: ${borrower}\n📂 โครงการ: ${project}\n📦 รายการ:\n${summaryText.join("\n")}`,
    );

    return {
      success: true,
      message: `บันทึกการยืมสำเร็จ ${successCount} รายการ`,
    };
  } catch (error) {
    return { success: false, message: error.toString() };
  } finally {
...
function testTelegram() {
  sendTelegramAlert(
    "🔔 <b>ทดสอบระบบแจ้งเตือน Stock</b>\nระบบเชื่อมต่อ Telegram สำเร็จเรียบร้อยแล้ว!",
  );
}
